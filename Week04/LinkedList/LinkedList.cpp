...
bool removeHead(List*& L)
{
    ...
    if (L->p_head == nullptr) {
        ...
        delete temp;
        if (L->p_head == nullptr) {
			L->p_tail = nullptr; // List is now empty   
        }
    }
    return true;
}
...
void removeTail(List*& L)
{
    ...
    if (L->p_head == L->p_tail) { // List have 1 node
        delete L->p_head;
        L->p_head = nullptr;
        L->p_tail = nullptr;
        return;
...
void removeBefore(List*& L, int val)
{
    ...
    if (L->p_head == nullptr || L->p_head->p_next== nullptr) {
        return; // List empty or 1 node
    }
    if (L->p_head->key == val) {
        return; // nốt đầu tiền là value nên ko cần xóa 
    }
    NODE* current = L->p_head;
    NODE* prev = nullptr;
    while (current->p_next != nullptr) {
        if (current->p_next->key == val) { 
            if (prev == nullptr) { // nốt đầu tiên là nốt cần xóa
                NODE* temp = L->p_head;
                L->p_head = L->p_head->p_next;
                delete temp;
            }
...
void removeAfter(List*& L, int val)
{
    // Your code here //
    if (L->p_head == nullptr || L->p_head->p_next == nullptr) {
        return; // trống or 1 node
    }
    if (L->p_tail->key == val) {
        return; // node cuối cùng chứa value
    }
    NODE* current = L->p_head;
    ...
    while (current-> p_next != nullptr ) {
        ...
        if (current->key == val) {
            if (temp->p_next == nullptr) { // nốt cuối cùng là nốt cần xóa
                delete temp;
...
bool addPos(List*& L, int data, int pos)
{
    // Your code here //
    if (pos < 0) {
        return false; // vị trí ko hợp lệ
    }
    NODE* newNode = createNode(data);
    if (L->p_head == nullptr) { // list trống
        if (pos != 0) {
            return false;
        }
        else {
    ...
    if (cnt == pos) { // trường hợp them vào cuối 
        prev->p_next = newNode;
        L->p_tail = newNode;
        return true;
    }
...
void removePos(List*& L, int data, int pos)
{
    ...
    while (current != nullptr) {
        if (cnt == pos) {
            if (prev == nullptr) { // xóa nốt đầu
                L->p_head = L->p_head->p_next;
                delete current;
                if (L->p_head == nullptr) { // sau khi xóa thì list trống
                    L->p_tail = nullptr;
                }
            }
    ...
    if (cnt < pos) {
		return; // vị trí lớn hơn số nốt trong list
    }
}
...
bool addBefore(List*& L, int data, int val)
{
    // Your code here //
    if (L->p_head == nullptr) {
        return false; //list trôngs
    }
    if (L->p_head->p_next == nullptr && L->p_head->key != val) {
        return false; // list 1 phần tử nhưng ko bằng val
    }
    NODE* newNode = createNode(data);
    if (L->p_head->p_next == nullptr && L->p_head->key == val) { // list 1 phần tử và bằng val 
        newNode->p_next = L->p_head;
        L->p_head = newNode;
    }
    NODE* current = L->p_head;
...
bool addAfter(List*& L, int data, int val)
{
    // Your code here //
    if (L->p_head == nullptr) {
        return false; // list trống 
    }
    NODE* newNode = createNode(data);
    if (L->p_head->p_next == nullptr ) {
        if (L->p_head->key != val) { // 1 phần tử và ko bằng val
            return false;
        }
		else { // 1 phần tử và bằng val
            L->p_tail->p_next = newNode;
            L->p_tail = newNode;
        }
        return true;
    }
    if (L->p_tail->key == val) { // val ở cuối 
        L->p_tail->p_next = newNode;
        L->p_tail = newNode;
        return true;
    }
    ...
    while (current != nullptr) { 
        if (current->key == val) { // val ở đầu or giữa 
...
List* reverseList(List* L)
{
    ...
    while (current != nullptr) {
        ...
        if (newList->p_tail == nullptr) { // thêm node lần đầu vào list
            newList->p_tail = newNode;
        }
        current = current->p_next;
     }
    return newList;
    ...
    while (current != nullptr) {
        ...
        while (runner != nullptr) {
            if (runner->key == current->key) {
                start->p_next = runner->p_next;
				if (runner == L->p_tail) { // xóa nốt cuối
					L->p_tail = start;
				}
                delete runner;
                runner = start->p_next;
...
bool removeElement(List*& L, int key)
{
    ...
    while (current != nullptr) {
        if (current->key == key) {
			removed = true;
            NODE* temp = current;
            if (prev == nullptr) { // key ở nốt đầu 
                
                L->p_head = current->p_next;
                current = L->p_head;
                delete temp;
                if (L->p_head == nullptr) { // sau khi xóa list trống
                    L->p_tail = nullptr;
                }
			}
            else { // key ở giữa và cuối 
               
                prev->p_next = current->p_next;
                current = current->p_next;
                if (temp == L->p_tail) { // key ở nốt cuối 
                    L->p_tail = prev;
                }
                delete temp;
            }
